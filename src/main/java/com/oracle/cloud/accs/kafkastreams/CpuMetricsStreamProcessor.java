...
public class CpuMetricsStreamProcessor implements Processor<String, String> {
    ...
    private final String redisTenantIdentifier;
    ...
    public void process(String machineID, String currentCPUUsage) {
        ...
        try {
            ...
            if (machineID != null && currentCPUUsage != null) {
                ...
                redis.lpush(redisTenantIdentifier + "-" + machineID, currentCPUUsage);
                redis.ltrim(redisTenantIdentifier + "-" + machineID, 0, 109); //110 elements

                System.out.println("Updated Machine list in Redis....");

                //2. push the cumulative avg to sorted set
                redis.zadd(redisTenantIdentifier + "-" + metricsRedisSortedSet, latestCumulativeAvg, machineID);
